...
namespace JoystickServerApp.HID
{
    public static class AsusHid
    {
        ...
        public static IEnumerable<HidDevice> FindDevices(byte reportId)
        {
            ...
            try
            {
                deviceList = loader.GetDevices(ASUS_ID).Where(device => deviceIds.Contains(device.ProductID)  && device.GetMaxFeatureReportLength() > 0);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error enumerating HID devices: {ex.Message}");
                yield break;
            }
        ...
        public static HidStream FindHidStream(byte reportId)
        {
            try
            {
                ...
                foreach (var device in devices)
                    Console.WriteLine($"Input available: {device.DevicePath} {device.ProductID.ToString("X")} {device.GetMaxFeatureReportLength()}");

                return devices.FirstOrDefault()?.Open();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error accessing HID device: {ex.Message}");
            }
            ...
            foreach (var device in FindDevices(INPUT_ID))
            {
                try
                {
                    using (var stream = device.Open())
                    {
                        ...
                        if (!(log is null)) Console.WriteLine($"{log} {device.ProductID.ToString("X")}|{device.GetMaxFeatureReportLength()}: {BitConverter.ToString(data)}");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error setting feature {device.GetMaxFeatureReportLength()} {device.DevicePath}: {BitConverter.ToString(data)} {ex.Message}");
                ...
                try
                {
                    using (var stream = device.Open())
                        foreach (var data in dataList)
                            ...
                            {
                                stream.Write(data);
                                if (!(log is null)) Console.WriteLine($"{log} {device.ProductID.ToString("X")}: {BitConverter.ToString(data)}");
                            }
                            catch (Exception ex)
                            {
                                if (!(log is null)) Console.WriteLine($"Error writing {log} {device.ProductID.ToString("X")}: {ex.Message} {BitConverter.ToString(data)} ");
                            }
                }
                catch (Exception ex)
                {
                    if (!(log is null)) Console.WriteLine($"Error opening {log} {device.ProductID.ToString("X")}: {ex.Message}");
                }
        }
